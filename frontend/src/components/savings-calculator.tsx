 ...
 */
const ASSUMPTIONS = {
  pcCostInPaise: 4_200_000, // ₹42,000 for a business desktop PC
  hostCostInPaise: 6_500_000, // ₹65,000 for a server that comfortably drives 10 seats
  seatsPerHost: 10,
  devicePriceInPaise: 2_890_000, // ₹28,900 for an RX420, which serves 2 seats
  seatsPerDevice: 2,
  pcWatts: 110,
  hostWatts: 250,
  ...
  daysPerYear: 250,
  years: 5,
  paisePerKwh: 800, // ₹8 per unit
};

function electricityInPaise(watts: number) {
  const kwh =
    (watts / 1000) * ASSUMPTIONS.hoursPerDay * ASSUMPTIONS.daysPerYear * ASSUMPTIONS.years;
  return Math.round(kwh * ASSUMPTIONS.paisePerKwh);
}
...
export function SavingsCalculator() {
  ...
  const result = useMemo(() => {
    ...
    const pcUpfront = seats * ASSUMPTIONS.pcCostInPaise;
    const pcPower = electricityInPaise(seats * ASSUMPTIONS.pcWatts);

    const ncUpfront =
      hosts * ASSUMPTIONS.hostCostInPaise + devices * ASSUMPTIONS.devicePriceInPaise;
    const ncPower = electricityInPaise(
    ...
    <Card>
      <CardBody className="space-y-6">
        ...
        <p className="text-xs leading-relaxed text-slate-500">
          Assumes {formatInr(ASSUMPTIONS.pcCostInPaise)} per desktop PC,{" "}
          {formatInr(ASSUMPTIONS.hostCostInPaise)} per host serving{" "}
          {ASSUMPTIONS.seatsPerHost} seats, {formatInr(ASSUMPTIONS.devicePriceInPaise)} per RX420
          serving 2 seats, {ASSUMPTIONS.hoursPerDay} hours a day for{" "}
          {ASSUMPTIONS.daysPerYear} days a year at ₹{ASSUMPTIONS.paisePerKwh / 100} per unit.
