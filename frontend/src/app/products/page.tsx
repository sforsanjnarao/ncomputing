...
const COMPARISON: { label: string; value: (product: Product) => string }[] = [
  { label: "Users per device", value: (p) => String(p.usersPerDevice) },
  { label: "Price per device", value: (p) => formatInr(p.priceInPaise) },
  {
    label: "Price per seat",
    value: (p) => formatInr(Math.round(p.priceInPaise / p.usersPerDevice)),
  },
  { label: "Where the desktop runs", value: (p) => p.specs["Works with"] ?? "On your own host PC" },
  { label: "Platform", value: (p) => p.specs.Platform ?? "—" },
];
