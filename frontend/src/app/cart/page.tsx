...
export default function CartPage() {
  const { lines, updateQuantity, remove, subtotalInPaise, taxInPaise, totalInPaise, itemCount } =
    useCart();
                  ...
                  <div className="flex-1">
                    ...
                    <ul className="mt-1 space-y-0.5">
                      {line.optionLabels.map((option) => (
                        <li key={option.group} className="text-sm text-slate-500">
                          {option.group}: {option.label}
                        </li>
                      ))}
                    </ul>
                    <p className="mt-2 text-sm text-slate-600">
                      {formatInr(line.unitPriceInPaise)} each
                    </p>
                  </div>
